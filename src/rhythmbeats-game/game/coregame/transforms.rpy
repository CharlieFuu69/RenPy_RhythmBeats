...
transform full_combo_line_effect(xymap = [(0.0, 0.0), 0.0], delta = 0.0):
    ...
    block:
        ...
        pause delta + 0.001
        ease 0.7 pos(xymap[0][1], xymap[1])
...
transform place_atl(xybef=(0.0, 0.0), xyaft=(0.0, 0.0), ref=(0.0, 0.0), delta=0.0, speed=0.3):
    subpixel True
    anchor(ref[0], ref[1])
    ...
    on show, start:
        pos(xybef[0], xybef[1])
        pause delta + 0.001
        easein_quint speed pos(xyaft[0], xyaft[1])

    on hide:
        pos(xyaft[0], xyaft[1])
        easeout_quint speed pos(xybef[0], xybef[1])
